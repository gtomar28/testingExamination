...
import { ItemCategoryPostApi } from '../Utils/Apis'
import { ItemCategoryGetAllApi } from '../Utils/Apis'
import { ItemCategoryDeleteApi } from '../Utils/Apis'
import { ItemCategoryGetByIdApi } from '../Utils/Apis'
import { ItemCategoryUpdateApi } from '../Utils/Apis'
...
const Item_category = () => {
  ...
  console.log('deleteee', IdForDelete)
  const [IdForUpdate, setIdForUpdate] = useState()
  const [showadd, setShowadd] = useState(true)
  const [hideedit, setHideedit] = useState(false)
  const [itemCategoryData, setItemCategoryData] = useState([])
  console.log('item category gdata ', itemCategoryData)
  const [myIncomeCategoryData, setMyIncomeCategoryData] = useState([])

  const [isValidCategoryNameRequired, setIsValidCategoryNameRequired] = useState(false);
  ...
  const FuncValidation = () => {
    ...
    // name 
    if (!itemCategory || itemCategory === "" || !/^[A-Za-z\s]+$/.test(itemCategory)) {
      setIsValidCategoryNameRequired(true)
      inValid = false
      setLoader(false)
    }
    else {
  ...
  const handleCategoryName = (e2) => {
    setItemCategory(e2);
    const nameRegex = /^[A-Za-z\s]+$/;
    setIsValidCategoryNameRequired(nameRegex.test(e2));
    if (e2 === "" || !nameRegex.test(e2)) {
      setIsValidCategoryNameRequired(true)
    } else {
      setIsValidCategoryNameRequired(false)
    }
  }
  ...
  const MyItemCategoryPostApi = async () => {
    if (FuncValidation()) {
      ...
      setLoader(true)
      try {
        const response = await ItemCategoryPostApi(formData);
  ...
  // Get All api  
  const MyItemCategoryGetAllApi = async () => {
    setLoader(true)

    try {
      const response = await ItemCategoryGetAllApi(pageNo, pageSize);
      console.log('my Item Category all dataaaaa', response);
      if (response?.status === 200) {
        // toast.success(response?.data?.message)
  ...
  // Delete api
  const MyItemCategoryDelApi = async (id) => {
    setLoader(true)

    try {
      const response = await ItemCategoryDeleteApi(id);
  ...
  //  Get by id 
  const MyItemCategoryGetById = async (id) => {
    setIdForUpdate(id)
    setLoader(true)
    try {
      const response = await ItemCategoryGetByIdApi(id);
      console.log('Itemmmm category Get By Idddddd', response)

      if (response?.status === 200) {
        // toast.success(response?.data?.msg)
        setItemCategory(response?.data?.itemCategory?.name)
        setDescription(response?.data?.itemCategory?.description)
  ...
  const MyItemCategoryPutApi = async () => {
    ...
    try {
      ...
      const response = await ItemCategoryUpdateApi(IdForUpdate, formData);
          ...
          <div className="breadCrum ms-2">
            <nav style={{ '--bs-breadcrumb-divider': "'>'" }} aria-label="breadcrumb">
              <ol className="breadcrumb ms-2">
                ...
                <li className="breadcrumb-item breadcrum-li heading-14" ><Link href="#">Item Category</Link></li>
              </ol>
            </nav>
          </div>

        </div>
        <h5 className='ms-3 mb-2 margin-minus22 heading-16' style={{ marginTop: '-12px' }}>Item Category</h5>
        ...
        <div className="main-content-conatainer pt-1 ">
          ...
          <div className="row p-3">
            ...
            <div className="col-lg-6 col-md-6 col-sm-12  ">
              <div className="mb-3" >
                <label for="exampleFormControlInput1" className="form-label label-color heading-14">Item Category *</label>
                <input type="text" className="form-control form-focus form-control-sm   heading-14" value={itemCategory} onChange={(e) => handleCategoryName(e.target.value)} style={{ marginTop: '-4px' }} id="exampleFormControlInput1" placeholder="Enter Category " />
              </div>
              <div className=''>
                {
                  isValidCategoryNameRequired && (
                    <p className='' style={{ color: 'red', fontSize: '14px', marginTop: '-15px' }}>Category Name required</p>
                  )
                }
              </div>
            </div>
              ...
              <div className=''>
                {
                  isValidDescriptionRequired && (
                    <p className='' style={{ color: 'red', fontSize: '14px', marginTop: '-15px' }}>Description required</p>
                  )
                }
              </div>
          ...
          <div className="row mb-3 buttons-topss">
            <div className='my-button11 heading-16'>
              <button type="button" class="btn btn-outline-success my-green" onClick={MyItemCategoryPostApi}>Add Item Category</button>
              <button type="button" class="btn btn-outline-success">Cancel</button>
            </div>
          </div>
          <div>
            <p className='p-2 ps-4 heading-18'>Item Category List</p>
          </div>
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  ...
                  <th style={{ width: '40%' }}>Item Category</th>
                    ...
                    <tr className='heading-14' >
                      ...
                      <td className=' greyText' >
                        <div className='d-flex '>
                          <Link className="dropdown-item " to={''} data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight1234" aria-controls="offcanvasRight1234" onClick={(e) => MyItemCategoryGetById(item.id)} >
        ...
        <div className="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight1234" aria-labelledby="offcanvasRightLabel" ref={offcanvasRef22}>
          {
            showadd && (
              <>
                ...
                <div className="offcanvas-body pt-0">
                  <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label heading-16">Item Category *</label>
                    <input type="text" class="form-control form-control-sm" id="exampleFormControlInput1" value={itemCategory} onChange={(e) => setItemCategory(e.target.value)} placeholder="Select Title" />
                  </div>
                  <div class="mb-3">
                  ...
                  <div className='my-button11 '>
                    <button type="button" className="btn btn-outline-success heading-16 btn-bgAndColor" onClick={MyItemCategoryPutApi} >Save</button>
                    <button type="button" className="btn btn-outline-success heading-16">Cancel</button>
                  </div>
                </div>
                ...
                <div className="offcanvas-body">
                  ...
                  <div className="sure-main-container mt-4">
                    <div className="sure-container">
                      ...
                      <div className="mt-4">
                        <button type="button" className="btn my-btn  button00 my-button112233RedDelete" disabled={forDelete ? false : true} onClick={(e) => MyItemCategoryDelApi(IdForDelete)} >Delete</button>
