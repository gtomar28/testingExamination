...
const BookListManager = () => {
  ...
  const FuncValidation = () => {
    let inValid = true;
    // name 
    if (!bookName || bookName === "" || !/^[A-Za-z\s]+$/.test(bookName)) {
    ...
    // Author name 
    if (!Author || Author === "" || !/^[A-Za-z\s]+$/.test(Author)) {
    ...
    // Number of copy 
    if (!NumberBook || NumberBook === "" || !/^\d{1,4}$/.test(NumberBook)) {
      setIsValidNoOfCopyRequired(true)
      inValid = false
      setLoader(false)
    }
    else {
  ...
  const handleNumberOfCopy = (e2) => {
    setNumberBook(e2);
    const noRegex = /^\d{1,4}$/;
    setIsValidNoOfCopyRequired(noRegex.test(e2));
    if (e2 === "" || !noRegex.test(e2)) {
      setIsValidNoOfCopyRequired(true)
    } else {
      setIsValidNoOfCopyRequired(false)
    }
  }
  ...
  const SubcPutDataApi = async () => {
    ...
    if (FuncValidation()) {
      ...
      formData.append('numberOfBooks', NumberBook);
      setLoader(true)
      try {
  ...
  const BookManGetByIdApi = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        setNumberBook(response?.data?.Books?.noOfCopies)
  ...
  const MyNoticePutApi = async (id) => {
    ...
    if (FuncValidation()) {
      ...
      try {
        ...
        formData.append('author', Author)
        formData.append('numberOfBooks', NumberBook)
        formData.append('availableBooks', NumberAvailableBook)
          ...
          <div className='d-flex g-1 for-media-query'>
            ...
            <div className='me-2 search-responsive'>
              <div className="input-group mb-3 search-responsive">
                ...
                <span className="input-group-text button-bg-color button-color heading-14 font-color " style={{ cursor: 'pointer', height: "34px" }} id="basic-addon2" onClick={MyRolPermisGetAllApi}>Search</span>
              </div>
            </div>
            <Link type="button" className="btn btn-success heading-16 my-own-button me-3" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop" aria-controls="staticBackdrop">+ Add Book</Link>
              ...
              <tbody className='heading-14 align-middle greyTextColor'>
                {
                  BookManagerData.map((item, index) => (
                    <tr className='heading-14' >
                      ...
                      <td className=' greyText pe-0'>{item.authorName}</td>
                      <td className=' greyText pe-0 my-anchor-view'>{item.noOfCopies}</td>
                      <td className=' greyText pe-0'>{item.availableCopies}</td>
                ...
                <div class="offcanvas-body">
                  <div className="input " >
                    <div className="mb-3" style={{ marginTop: '-4px' }}>
                      ...
                      <input type="email" className="form-control form-focus  label-color heading-14" onChange={(e) => handleName(e.target.value)} style={{ marginTop: '-4px' }} id="exampleFormControlInput1" placeholder="Enter Book Name" />
                    </div>
                    <div className='pt-1'>
                      {isValidNameRequired && (
                    ...
                    <div className="mb-3" style={{ marginTop: '-4px' }}>
                      ...
                      <input type="email" className="form-control form-focus  label-color heading-14" onChange={(e) => handleAuthorName(e.target.value)} style={{ marginTop: '-4px' }} id="exampleFormControlInput1" placeholder="Enter Author Name" />
                    </div>
                    <div className='pt-1'>
                    ...
                    <div className="mb-3" style={{ marginTop: '-6px' }}>
                      ...
                      <input type="email" className="form-control form-focus  label-color heading-14" onChange={(e) => handleNumberOfCopy(e.target.value)} style={{ marginTop: '-4px' }} id="exampleFormControlInput1" placeholder="Number of Copy" />
                    </div>
                    <div className='pt-1'>
            ...
            <>
              <div className="offcanvas-end offcanvas" data-bs-backdrop="static" tabindex="-1" id="staticBackdrop1234" aria-labelledby="staticBackdropLabel" ref={offcanvasRef22}>
                <div className="offcanvas-header">
                  ...
                  <h5 className="offcanvas-title heading-16" id="offcanvasRightLabel"> Edit Add Book</h5>
                </div>
                <hr className='mx-3' style={{ marginTop: '-3px' }} />
