...
import { NoticePostApi } from '../Utils/Apis'
import { NoticeGetAllApi } from '../Utils/Apis'
import { NoticeDeleteApi } from '../Utils/Apis'
...
const Notice = () => {
  ...
  const EditHandleBtn = (e) => {

    if (editshow === true && edithide === false) {
      setEditshow(false)
      setEdithide(true)
    } else {
      setEditshow(true)
  ...
  const AddHandleBtn = (e) => {

    if (addshow === true && addhide === false) {
      setAddshow(false)
      setAddhide(true)
    } else {
      setAddshow(true)
  ...
  const MyNoticePostApi = async () => {
    ...
    if (FuncValidation()) {
      ...
      try {
        const response = await NoticePostApi(formData);
  ...
  const MyNoticeGetAllApi = async () => {
    setLoader(true)
    try {
      const response = await NoticeGetAllApi(searchKey, pageNo, pageSize);
  ...
  const MyNoticeDeleteApi = async (id) => {
    setLoader(true)
    try {
      const response = await NoticeDeleteApi(id);
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  ...
                  <th style={{ width: '680px' }}>Notice Details</th>
