...
import { TeacherNoticePostApi } from '../Utils/Apis'
import { TeacherNoticeGetAllApi } from '../Utils/Apis'
import { TeacherNoticeCSV } from '../Utils/Apis'
import { TeacherNoticeDeleteApi } from '../Utils/Apis'
import { TeacherNoticeGetByIdApi } from '../Utils/Apis'
import { TeacherNoticePutApi } from '../Utils/Apis'
...
const Notice = () => {
  ...
  const [isValidDscriptionRequired, setIsValidDscriptionRequired] = useState(false);
  const [csvData, setCsvData] = useState([]);

  const Download_Slip = async () => {
    try {
      const response = await TeacherNoticeCSV();
  ...
  const FuncValidation = () => {
    let isValid = true;
    // name 
    if (!noticeHeading || noticeHeading === "" || !/^[A-Za-z\s]+$/.test(noticeHeading)) {
    ...
    // description
    if (!noticeDescription || noticeDescription === "" || !/^[A-Za-z\s]+$/.test(noticeDescription)) {
      setIsValidDscriptionRequired(true)
      isValid = false;
      setLoader(false)
    }
    else {
  ...
  const handleDescription = (e2) => {
    setNoticeDescription(e2);
    const nameRegex = /^[A-Za-z\s]+$/;
    setIsValidDscriptionRequired(nameRegex.test(e2));

    if (e2 === "" || !nameRegex.test(e2)) {
      setIsValidDscriptionRequired(true)
    } else {
      setIsValidDscriptionRequired(false)
    }
  }
  ...
  const MyNoticePostApi = async () => {
    ...
    if (FuncValidation()) {
      ...
      setLoader(true)
      try {
        const response = await TeacherNoticePostApi(formData);
  ...
  const MyNoticeGetAllApi = async () => {
    setLoader(true)
    try {
      const response = await TeacherNoticeGetAllApi(searchKey, pageNo, pageSize);
      console.log('NOTICE get All Api data', response);
      if (response?.status === 200) {
        // toast.success(response?.data?.classes?.msg)
        setNoticeAllData(response?.data?.notices)
  ...
  const MyNoticeDeleteApi = async (id) => {
    setLoader(true)
    try {
      const response = await TeacherNoticeDeleteApi(id);
  ...
  const MyNoticeGetByIdApi = async (id) => {
    setNoticeIdForUpdate(id)
    setLoader(true)
    try {
      const response = await TeacherNoticeGetByIdApi(id);
      ...
      if (response?.status === 200) {
        ...
        setNoticeDescription(response?.data?.notice?.description)
        setLoader(false)
      } else {
        toast.error(response?.data?.message);
      }
  ...
  const MyNoticePutApi = async (id) => {
    if (FuncValidation()) {
      ...
      try {
        const formData = new FormData()
        formData.append('noticeTitle', noticeTitleById)
        formData.append('noticeDescription', noticeDescriptionById)

        const response = await TeacherNoticePutApi(id, formData);
  ...
  const IdTransfer = (id) => {
    navigate("/noticeview", { state: { id: id } })
  }
  const handleForDelete = () => {
    MyNoticeDeleteApi(noticeIdForDelete)
  }
  return (
    <Container>
      {
      ...
      <div className="container-fluid main-body p-3">
          ...
          <div className='d-flex g-1 for-media-query'>
            ...
            <div className='me-2 search-responsive'>
              <div className="input-group mb-3 ">
                <input type="text" className="form-control form-focus font-color" style={{ height: '34px' }} placeholder="Search Notice" aria-label="Recipient's username" onChange={(e) => setsearchKey(e.target.value)} aria-describedby="basic-addon2" />
                <span className="input-group-text button-bg-color button-color heading-14 font-color " style={{ cursor: 'pointer', height: "34px" }} id="basic-addon2" onClick={MyNoticeGetAllApi}>Search</span>
              </div>
            </div>
            <Link type="button" className="btn btn-success heading-16 my-own-button me-3" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop1012" aria-controls="staticBackdrop"  >+ Add Notice</Link>
          </div>

        </div>
        <h5 className='ms-3 mb-2 margin-minus22 heading-16 heading-responsive' style={{ marginTop: '-22px' }}>Notice Details</h5>
            ...
            <table className="table table-sm table-striped">
              <thead className=''>
                <tr className='heading-16 text-color-000' style={{ fontWeight: '500' }}>
                  ...
                  <th style={{ width: '60%' }}>Notice Details</th>
              ...
              <tbody className='heading-14 align-middle greyTextColor'>
                {
                  noticeAllData.map((item, index) => (
                    <tr className='heading-14' >
                      <td className=' greyText pe-0'>{index + 1}</td>
                      <td className=' greyText pe-0'>{item.noticeTitle}</td>
                      <td className=' greyText pe-0'>{item.noticeDate}</td>
        ...
        {/* ################## Edit Off Canvas Area end ####################  */}
        {
          editshow && (
            <>
              <div className="offcanvas-end offcanvas" data-bs-backdrop="static" tabindex="-1" id="staticBackdrop" aria-labelledby="staticBackdropLabel" ref={offcanvasRef22}>
                <div className="offcanvas-header">
                  ...
                  <h5 className="offcanvas-title heading-16" id="offcanvasRightLabel">Edit Notice</h5>
                </div>
...

        {
          addshow && (
            <>
              <div className="offcanvas-end offcanvas" data-bs-backdrop="static" tabindex="-1" id="staticBackdrop1012" aria-labelledby="staticBackdropLabel" ref={offcanvasRef}>
                <div className="offcanvas-header">
                  <Link data-bs-dismiss="offcanvas" >
                    <svg width="28" height="15" viewBox="0 0 28 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M8.06 0.295798C8.15373 0.388761 8.22812 0.499362 8.27889 0.621222C8.32966 0.743081 8.3558 0.873786 8.3558 1.0058C8.3558 1.13781 8.32966 1.26852 8.27889 1.39038C8.22812 1.51223 8.15373 1.62284 8.06 1.7158L3.46 6.3158H27C27.2652 6.3158 27.5196 6.42115 27.7071 6.60869C27.8946 6.79623 28 7.05058 28 7.3158C28 7.58102 27.8946 7.83537 27.7071 8.0229C27.5196 8.21044 27.2652 8.3158 27 8.3158H3.48L8.06 12.8858C8.24625 13.0732 8.35079 13.3266 8.35079 13.5908C8.35079 13.855 8.24625 14.1084 8.06 14.2958C7.87264 14.482 7.61918 14.5866 7.355 14.5866C7.09081 14.5866 6.83736 14.482 6.65 14.2958L0.289999 7.9358C0.204397 7.85367 0.136286 7.75508 0.089756 7.64596C0.0432262 7.53683 0.0192413 7.41943 0.0192413 7.3008C0.0192413 7.18217 0.0432262 7.06476 0.089756 6.95564C0.136286 6.84652 0.204397 6.74793 0.289999 6.6658L6.64 0.295798C6.73296 0.20207 6.84356 0.127676 6.96542 0.0769072C7.08728 0.0261385 7.21799 0 7.35 0C7.48201 0 7.61272 0.0261385 7.73458 0.0769072C7.85643 0.127676 7.96704 0.20207 8.06 0.295798Z" fill="#008479" />
                ...
                <div class="offcanvas-body">
                  <div className="input " >
                    <div className="mb-3" style={{ marginTop: '-6px' }}>
                      <label for="exampleFormControlInput1" className="form-label label-color heading-14">Notice heading</label>
                      <input type="email" className="form-control form-focus   heading-14" onChange={(e) => handleName(e.target.value)} style={{ marginTop: '-4px' }} id="exampleFormControlInput1" placeholder="Enter Notice Heading " />
                    </div>
                    <div className='pt-1'>
                      {isValidNameRequired && (
                        <p className='ms-1' style={{ color: 'red', fontSize: '14px', marginTop: '-18px' }}>
                    ...
                    <div className="mb-3" style={{ marginTop: '-6px' }}>
                      <label for="exampleFormControlInput1" className="form-label label-color heading-14">Description</label>
                      <textarea class="form-control heading-14 px-4" id="exampleFormControlTextarea1" rows="5" onChange={(e) => handleDescription(e.target.value)} placeholder='Enter Notice Heading'>
                      </textarea>
                    </div>
                    <div className='pt-1'>
                      {isValidDscriptionRequired && (
